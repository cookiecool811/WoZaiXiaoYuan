...
7. **按日期自动判断签到时机**：仅在每周日晚上和法定节假日最后一天晚上自动签到，其余日期自动跳过，无需手动修改定时规则
...
- [x] 2026.9.20 新增按日期自动判断签到时机功能：仅每周日及法定节假日最后一天自动签到，其余日期自动跳过；新增 chinese-calendar 依赖。
...
- 在下方的`Environment secrets`中新增Secret，所需Secret有：
  ...
  **修改打卡时间/配置多用户**
  - 点击.github -> workflows -> punch.yml，已配置为每天北京时间 20:00（UTC 12:00）自动执行，代码内部会自动判断当天是否为周日或法定节假日最后一天，非签到日会直接跳过，无需手动修改 cron 时间。如果需要多用户打卡，将属于User02的部分取消注释后，在Environment中新增一个名为`CONFIG_02`的环境后，填入同样的Secert即可
